...
const Cart = ()=>{
    ...
    return (
        ...
        <div className="cart bg-gray-200  py-10 h-screen px-14 flex justify-between">
        ...
        {cartItems.length===0 && (<div className="bg-white p-5 h-80 flex justify-center items-center flex-col">
            ...
            {cartItems.map((item)=>(
                <div key={item.card.info.id} className="Head   bg-white p-5 flex ">

                    <img src={MENU_ITEMS_IMAGE + item.card.info.imageId} className="w-14 h-14"/>
                    
                    <h1 className="font-bold ml-3"> {item.card.info.name} <br/>
                    <span className="text-zinc-400">₹{item.card.info.price?  item.card.info.price/100 : item.card.info.defaultPrice /100}</span>
                    </h1>
                    <div className="  ml-36 flex items-end p-2  w-8 h-4 mt-11">
                    <button className=" text-red-600  p-1" onClick={handleRemoveItem}>X</button>
                    </div>
